...
export function ChartsPanel({
  ...
  return (
    <div className={`bg-slate-50 dark:bg-slate-800/50 rounded-lg overflow-hidden ${className}`}>
      {/* View toggle buttons */}
      <div className="p-1 bg-slate-100 dark:bg-slate-800 rounded-lg m-2 sm:m-3 flex gap-1">
        {viewButtons.map((btn) => (
          <button
            key={btn.value}
            type="button"
      ...
      <div className="px-2 sm:px-3 pb-2 sm:pb-3">
        {view === 'altitude' && (
          <div>
            <p className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 mb-2 text-center">
